...
func run() -> void:
	...
	title(stage, "WALK  /  STRIDE > PASSING > STRIDE > PASSING > STRIDE", Vector2(18, 44), 11)
	...
	for row in 2:
		...
		for col in count:
			...
			for i in 80:
				actor.rig.gait.phase = col / (4.0 if row == 0 else 10.0)
			...
			title(stage, "F%02d" % (1 + col * (6 if row == 0 else 2)), Vector2(actor.position.x - 10, y + 9), 9)
	await process_frame
